...
from util.datasets import build_dataset, build_transform, IP102
...
def get_args_parser():
    ...
    parser.add_argument('--global_pool', action='store_true', default=False, help='Use global average pooling')
...
def main(args):
    ...
    if (not args.distributed or args.rank == 0) and args.log_dir is not None and not args.eval:
    ...
    if args.finetune and not args.eval:
        ...
        for k in checkpoint_model:
            if k.startswith('img_encoder.'):
                new_k = k.replace('img_encoder.', '')
                new_state_dict[new_k] = checkpoint_model[k]
            else:
                new_state_dict[k] = checkpoint_model[k]
